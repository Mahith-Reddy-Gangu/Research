...
Each subproject reads paths and hyperparameters from its own `config.yaml`. Every
run records its commit via `git_provenance.py` (writes `GIT_SHA.txt` into the run
directory) — a dirty working tree is flagged loudly, since such a run is not
reproducible.
